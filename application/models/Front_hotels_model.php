...
class Front_hotels_model extends CI_Model {
	...
	public function filter() {
		$this->db->select('slug_'.$this->input->post('lang').', title_'.$this->input->post('lang').', category, date, image, price');
		$this->db->from('hotels');
	...
	public function home_hotels($limit = null) {
		$this->db->select('*');
		$this->db->from('hotels');
		$this->db->order_by('date', 'DESC');
	...
	public function hotels_categories() {
