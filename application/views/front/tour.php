                                ...
                                <?php if (count($data['category']) && !empty($data['category'])): ?>
                                    <div class="news-card-tags-wrap">
                                        ...
                                        <?php  foreach ($data['category'] as $value): ?>
                                            ...
                                            <a href="/tours?cat=<?=$value['id']?>" class="news-card-tags" style="color: #000;">
                                                <?=$value['title_'.LANG]?>
                                            </a>
                                        <?php endforeach ?>
                                    </div>
                ...
                <div class="related-news">
                    <header class="block-header">
                        ...
                        <div class="block-header-actions">
                            <a href="<?=base_url('tours')?>" class="block-header-btn"><?=$translate['all']?></a>
                        </div>
                    </header>
                    <div>
                        <?php foreach ($latest_tours as $value): ?>
                        <div class="announces-card">
                            <h3 class="announces-card-title">
                                <a href="<?=base_url('tour/index/').$value['slug_'.LANG]?>"><?=$value['title_'.LANG]?></a>
                            </h3>
                            <div class="news-card-tags-wrap">
                                <?php foreach($value['categories'] as $row): ?>
                                    <a href="<?=base_url()?>tours?cat=<?=$row['id']?>" class="news-card-tags" style="color: #<?=$row['color']?>;">
                                        <?=$row['title_'.LANG]?>
                                    </a>
                                <?php endforeach; ?>
                            </div>
