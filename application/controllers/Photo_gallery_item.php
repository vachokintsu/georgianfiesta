...
class Photo_gallery_item extends CI_Controller {
	
	function __construct() {
		parent::__construct();
		$this->load->model(array('Front_model', 'Front_photo_gallery_model'));
	}

	public function index($slug = null)
	{
		if($slug === null) redirect(base_url()); 
		$data['data'] = $this->Front_photo_gallery_model->getBySlug(urldecode($slug));

		if($data['data']['slug'] != urldecode($slug)) {
			redirect(site_url('/photo_gallery/index/'.$data['data']['slug'].'?lang='.LANG));
		}
		...
		$this->load->view('front/photo_gallery_item', $data);
	}
