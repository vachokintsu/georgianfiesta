...
class Tv_item extends CI_Controller {
	
	function __construct() {
		parent::__construct();
		$this->load->model(array('Front_model', 'Front_tv_model', 'Front_announcements_model'));
	}

	public function index($slug = null)
	{
		if($slug === null) redirect(base_url()); 
		$data['data'] = $this->Front_tv_model->getBySlug(urldecode($slug));

		if($data['data']['slug'] != urldecode($slug)) {
			redirect(site_url('/tv_item/index/'.$data['data']['slug'].'?lang='.LANG));
		}
		...
		$this->load->view('front/tv_item', $data);
	}
